...
各単語の左に付いている青い矢印は、その単語をインポートするという意味である。
単語を1つ選んで「インポートしない」ボタンをクリックすると、青い矢印が無色に変わり、その単語をインポートしないことになる。

そのファイルを作った人の単語登録を信用している場合は、すべての単語が青矢印になっている状態で「決定」ボタンをクリックする。
そうでもない場合は、1個ずつの単語設定を見ながらインポートするかしないかを選んでから、最後に「決定」ボタンをクリックする。

最も信用できるファイルの単語辞書を最後にインポートすれば、良くない単語登録はすべて上書きされる。
...
位置を間違えた場合はもう一度 `Alt P` するとページ番号位置の設定が消える。
{: .note}
