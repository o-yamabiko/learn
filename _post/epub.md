...
: この仕様はテキストデータの仕様から来たもので、EPUB形式には Daisy Consortium が推奨する便利な方法が別にある。文の途中にページ番号が見えていると、読む際には煩わしいので、 Daisy Consortium の方法を採用する。具体的には以下のようにする。
...
p {text-indent: 1em;}
```
: `<p></p>`の中に`<aside></aside>`が入っているとEPUB規格に反するため、 `epubcheck` を通らない。解決方法は2種類考えられる。
1. 納品前の修正で、`xhtml`内の`<aside></aside>`を`<p></p>`の直後に移動し、`smil`内の順番を変えないようにすれば、読み上げ順序を変えずに`aside`を`p`の外に置けるはず。
