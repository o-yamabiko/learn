...
: これらのどちらもEPUBの読み飛ばし機能を設定できないので、手作業で入れた。
: 今後、マルチメディアデイジーの作成を続けるために、納品前の修正を自動的にする方法、それに合わせて効率的にグループ作業するための方法を考え中。
...
その代わり、 ChattyInfty3 AITalk版 の最大の難点である、日本語に無い文字の処理について、特別な対処をしなければいけない（SAPI5版を使う場合はこの問題が起こらないが、その場合は日本語の読み上げ調整ができない）。

 ChattyInfty3 AITalk版 内部の読み上げ処理は UTF-8 ではなく Shift_JIS で行われているため、日本語にない文字の合成音声による読み上げができない。また、書誌情報に日本語にない文字があると文字化けする。

対処方法として、以下の2点を採用する。

- 正しい外国語の読み上げをSAPI5版で作成して音声ファイルで当てる。
...
以下では、国立国会図書館の仕様書から参考にした部分と、それに対応するやまびこの仕様案を列挙する。
...
```
やまびこ仕様
: ①は不要。太字、斜体、傍点（圏点）、下線はEPUBで表現でき、音訳でもそれなりに表現する。単なるテキストデータと違い、省略する必要がない。再生機がそれを省略する場合があるとしても、それは利用者が自分の使用環境として把握しておくべきことで、提供データの方で省略する必要はない。
: ②は不要。単なるテキストデータと違い、EPUBで利用者に使いやすいページ付けができるので、ページ付けにこの方法を採用しない。
: ③④は、注の対象と注が離れた位置にある場合に必要。
: ⑤は数式を使う場合に必要。
...
: 音訳の慣習として、漢字説明程度の短い注では「音訳者注」を言わない。テキスト上では〔全角亀甲括弧〕によって区別できているし、音声についても音訳技術で区別できていれば良いと考える。
...
```
やまびこ仕様
: ページ番号をフレーズの途中に入れても良いか？  
    →フレーズの途中には入れない。原本改ページ後に最初に始まるフレーズの直前にページ番号を入れる。  
    理由：フレーズの途中に入れたページ番号が Media Overlays でうまく働くかどうか確認した結果、フレーズの途中に入れたページに飛ぼうとすると、そのページ番号を含むフレーズの冒頭から読み上げられる。それは前ページの最後のフレーズなので、音声での利用者にとっては、ページ開始位置が原本とも音声デイジーとも異なることになる。これは良くないので、音声デイジーの慣習に合わせる。
: 国立国会図書館のページ仕様はテキストデータの仕様から来たもので、EPUB形式には Daisy Consortium が推奨する便利な方法が別にある。文の途中にページ番号が見えていると、読む際には煩わしいので、 Daisy Consortium の方法を採用する。具体的には以下のようにする。
: ページ開始位置に該当する箇所に以下の形を挿入する。これは24ページの例。
```html
   <span
       epub:type="pagebreak"
       role="doc-pagebreak"
       ...
       aria-label="24"/>
```
 ChattyInfty3 編集上は、これ自体がフレーズを区切る性質があるので、改ページの直前直後にはフレーズ区切り`Ctrl + Shift + /`が必要ない（有っても無視される）。  
...
```
やまびこ仕様
: `opf` に WCAG の `schema` を入れる。 Ace by Daisy で確認する必要がある。必要な Dublin Core については「補足資料3」に従う。
...
: DAISY2.02 の製作慣習と違って、深い階層のセクションを浅く直す必要がないので、できるだけ原本の構造を反映したセクション構造にする。  
    ...
    DAISY 2.02 の2003年追加事項、DAISY 3 、EPUB 3 の仕様に従う場合、グループを使わないのでこの懸念がない。利用者は、深い階層を使いたくなければ使わなければ良いだけである。
: `<p></p>`の中には`<aside></aside>`を入れられないので、音訳者注の挿入時に工夫する必要がある（[「11. 7 段落」に対するやまびこ仕様](#117danraku)を参照）。
...
```
やまびこ仕様
: ChattyInfty3 や Plextalk Producer ではこれらを正しく指定できないので、納品前のアクセシブル化の際に修正する必要がある。
...
```
やまびこ仕様
: `<p></p>`の中に`<aside></aside>`が入っているとEPUB規格に反するため、 `epubcheck` を通らない。解決方法は2種類考えられる。利用者の使い勝手を考えると、できれば後者を採用したい。
1. 納品前の修正で、`xhtml`内の`<aside></aside>`を`<p></p>`の直後に移動し、`smil`内の順番を変えないようにすれば、読み上げ順序を変えずに`aside`を`p`の外に置ける。
1. `<p></p>`を`<div></div>`に置き換えれば、音訳者注の表示位置も読み上げ順序も変えずにEPUB規格に準拠できる。
: 段落には`<p></p>`か`<div class="paragraph"></div>`を使う。原本の字下げとして原稿に全角空白がついていれば削除し、代わりにスタイルシートで指定：
```css
p {text-indent: 1em;}
...
: 外字を作らない・使わない。
: 読み上げでどう処理するか？  
    → 漢字説明の必要な場合以外は、上の規則に従って書き込むだけで、何事もなかったように原本通りに読み上げる。
