...
「チュー」という読みを「ナカ」に変えたくても、この文書中の他の部分に「チュー」と読むべき所がある場合は単語登録`Ctrl J`しない。
...
ふりがなはAITalkの読み上げ対象ではないので、ふりがなの単語登録は無駄な作業となる。必要であれば、ふりがなの付いている元の表記の方を単語登録`Ctrl J`する。

![単語登録画面、ふりがな](media/ctusin/tango_furigana.png){: .naka}

### 数値を含む読みはできるだけ単語登録しない。
...
単語登録でアクセントの高い部分が2箇所以上必要になるのは、登録しようとする語句に、文節が2つ以上含まれる場合だけである。
