...
読み上げのアクセントがおかしい場合、アクセント自体をいじらずに、前後に「フレーズ区切り」や「ポーズ」を挿入するだけで改善されることがある。修正方法の優先順位として、以下の順番を念頭に置くと効率が良い。
...
AITalkが判断して区切った各フレーズは、読み上げ時に緑色にハイライトされる。そのフレーズを分割してマを取りたい場合は、その場所に「ハイライト分割 (Ctrl+Shift+/)」を入れる。
...
- ページ番号付けは、特に利用者から要求されていなければ省略する。ChattyInftyは本来、電子書籍を作るアプリケーションであり、電子書籍には「ページ番号」という概念がないので、ページ番号付けを自動的に行う機能がない。
